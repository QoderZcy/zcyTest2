...
本系统采用现代化的Spring Boot架构，通过多种设计模式的综合应用，实现了高内聚、低耦合的软件架构。系统主要展现了MVC分层架构、依赖注入、事务管理、缓存策略、配置管理和异常处理等核心设计模式。特别地，系统采用了无状态架构设计，通过外部化配置和标准化接口实现松耦合的系统集成。
...
**图表来源**
- [PhotoController.java:35-44](file://src/main/java/com/photo/controller/PhotoController.java#L35-L44)
- [PhotoService.java:38-46](file://src/main/java/com/photo/service/PhotoService.java#L38-L46)
- [PhotoRepository.java:19-20](file://src/main/java/com/photo/repository/PhotoRepository.java#L19-L20)

### 表现层设计

Controller层负责HTTP请求的接收和响应的生成，采用RESTful设计原则：

- **无状态设计**: Controller保持无状态特性，通过参数传递实现状态管理
- **统一响应格式**: 通过ApiResponse统一返回格式，支持无状态的前后端交互
- **参数验证**: 在Controller层进行基础参数验证，确保业务层专注核心逻辑

**章节来源**
- [PhotoController.java:35-316](file://src/main/java/com/photo/controller/PhotoController.java#L35-L316)
...
**章节来源**
- [PhotoService.java:38-385](file://src/main/java/com/photo/service/PhotoService.java#L38-L385)
...
**图表来源**
- [PhotoController.java:36-43](file://src/main/java/com/photo/controller/PhotoController.java#L36-L43)
- [PhotoService.java:38-46](file://src/main/java/com/photo/service/PhotoService.java#L38-L46)
...
**章节来源**
- [PhotoService.java:50-111](file://src/main/java/com/photo/service/PhotoService.java#L50-L111)
- [PhotoService.java:190-234](file://src/main/java/com/photo/service/PhotoService.java#L190-L234)
- [PhotoService.java:237-250](file://src/main/java/com/photo/service/PhotoService.java#L237-L250)
...
**图表来源**
- [CacheConfig.java:15-54](file://src/main/java/com/photo/config/CacheConfig.java#L15-L54)
- [PhotoService.java:141-160](file://src/main/java/com/photo/service/PhotoService.java#L141-L160)
...
**章节来源**
- [PhotoService.java:192-234](file://src/main/java/com/photo/service/PhotoService.java#L192-L234)
...
**章节来源**
- [CacheConfig.java:22-53](file://src/main/java/com/photo/config/CacheConfig.java#L22-L53)
...
**图表来源**
- [FileStorageProperties.java:14-94](file://src/main/java/com/photo/config/FileStorageProperties.java#L14-L94)
- [SecurityProperties.java:14-53](file://src/main/java/com/photo/config/SecurityProperties.java#L14-L53)
...
**章节来源**
- [application.yml:50-126](file://src/main/resources/application.yml#L50-L126)
...
**图表来源**
- [GlobalExceptionHandler.java:20-140](file://src/main/java/com/photo/exception/GlobalExceptionHandler.java#L20-L140)
...
**章节来源**
- [GlobalExceptionHandler.java:23-140](file://src/main/java/com/photo/exception/GlobalExceptionHandler.java#L23-L140)
...
系统采用无状态架构设计，通过以下原则实现松耦合和高可扩展性：

```mermaid
graph TB
subgraph "无状态架构特征"
Stateless[无状态设计<br/>无会话状态]
Idempotent[幂等性<br/>重复请求安全]
Cacheable[可缓存性<br/>数据可缓存]
Externalized[外部化<br/>配置可外部化]
Standardized[标准化<br/>接口标准化]
end
subgraph "实现机制"
Config[配置外化<br/>application.yml]
Security[安全防护<br/>防盗链、XSS]
Cache[缓存策略<br/>Caffeine本地缓存]
Exception[异常处理<br/>统一错误响应]
end
Stateless --> Config
Idempotent --> Security
Cacheable --> Cache
Externalized --> Exception
Standardized --> Stateless
```
...
1. **参数驱动状态**: 通过URL参数、请求体参数传递状态信息
2. **外部化配置**: 所有配置通过application.yml集中管理
3. **标准化接口**: RESTful API设计，支持幂等操作
4. **缓存无状态**: 使用Caffeine本地缓存替代会话缓存
...
Client->>Controller : 请求访问
Controller->>Security : 读取配置
Controller->>Utils : XSS防护
Controller->>Controller : 防盗链检查
Controller-->>Client : 返回响应
Note over Controller,Security : 无状态安全检查
```

**图表来源**
- [PhotoController.java:92-97](file://src/main/java/com/photo/controller/PhotoController.java#L92-L97)
- [SecurityProperties.java:19-36](file://src/main/java/com/photo/config/SecurityProperties.java#L19-L36)

### 无状态优势

- **水平扩展**: 无状态设计支持多实例部署和负载均衡
- **简化运维**: 无需会话粘滞，支持动态扩缩容
- **提高可用性**: 单点故障不影响整体系统运行
- **降低复杂度**: 移除会话管理的复杂性
...
**章节来源**
- [SecurityProperties.java:19-36](file://src/main/java/com/photo/config/SecurityProperties.java#L19-L36)
- [application.yml:120-145](file://src/main/resources/application.yml#L120-L145)

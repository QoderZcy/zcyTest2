...
## 更新摘要
**所做更改**
- 更新了系统架构概览，反映移除认证和安全系统的变更
- 修改了MVC分层架构详解，去除认证和授权相关的内容
- 更新了核心组件分析，移除SecurityConfig和AuthController相关内容
- 修订了数据流处理机制，反映无状态架构设计
- 更新了依赖注入与设计模式，调整组件关系图
- 修改了异常处理机制，移除认证相关异常处理
...
本项目是一个基于Spring Boot的图片上传与管理系统，采用经典的MVC分层架构设计。系统提供了完整的图片上传、存储、管理和访问功能，包括文件验证、缩略图生成、图片压缩等高级特性。
...
系统采用经典的三层MVC架构，结合领域驱动设计原则，实现了清晰的职责分离和良好的可扩展性。**更新**：架构已调整为无状态设计，移除了完整的认证和安全系统。
...
**图表来源**
- [PhotoController.java:31-316](file://src/main/java/com/photo/controller/PhotoController.java#L31-L316)
...
- [FileStorageService.java:23-300](file://src/main/java/com/photo/service/FileStorageService.java#L23-L300)
...
- [PhotoService.java:38-43](file://src/main/java/com/photo/service/PhotoService.java#L38-L43)
...
- [PhotoController.java:58-60](file://src/main/java/com/photo/controller/PhotoController.java#L58-L60)
...
**章节来源**
- [PhotoService.java:50-385](file://src/main/java/com/photo/service/PhotoService.java#L50-L385)
...
**章节来源**
- [FileStorageService.java:59-300](file://src/main/java/com/photo/service/FileStorageService.java#L59-L300)
...
**章节来源**
- [PhotoRepository.java:24-111](file://src/main/java/com/photo/repository/PhotoRepository.java#L24-L111)
...
ParamValid --> |是| ControllerCall["调用Controller方法"]
...
**图表来源**
- [PhotoController.java:48-314](file://src/main/java/com/photo/controller/PhotoController.java#L48-L314)
- [PhotoService.java:302-385](file://src/main/java/com/photo/service/PhotoService.java#L302-L385)
...
**图表来源**
- [PhotoService.java:50-111](file://src/main/java/com/photo/service/PhotoService.java#L50-L111)
- [FileStorageService.java:59-165](file://src/main/java/com/photo/service/FileStorageService.java#L59-L165)
...
**图表来源**
- [GlobalExceptionHandler.java:22-139](file://src/main/java/com/photo/exception/GlobalExceptionHandler.java#L22-L139)
...
| **更新**：移除了AccessDeniedException的处理 |
...
**章节来源**
- [application.yml:45-49](file://src/main/resources/application.yml#L45-L49)
- [PhotoService.java:141-141](file://src/main/java/com/photo/service/PhotoService.java#L141-L141)
...
**更新**：系统已演进为无状态架构设计，移除了完整的认证和安全系统，简化了部署和运维复杂度，同时保留了核心的文件管理和访问功能。
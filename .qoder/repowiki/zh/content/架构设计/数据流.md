...
## 更新摘要
**所做更改**
- 更新了上传流程数据流，移除了用户认证和授权相关步骤
- 更新了下载流程数据流，移除了用户权限验证步骤
- 更新了断点续传流程数据流，移除了用户权限验证步骤
- 更新了组件间交互关系，移除了认证相关组件
- 更新了安全配置说明，强调防盗链机制而非用户认证
...
**重要更新**：系统现已移除用户认证和授权流程，简化为基于文件名的访问控制和防盗链保护机制。
...
- [PhotoService.java:35-385](file://src/main/java/com/photo/service/PhotoService.java#L35-L385)
- [FileStorageService.java:22-300](file://src/main/java/com/photo/service/FileStorageService.java#L22-L300)
- [PhotoRepository.java:19-112](file://src/main/java/com/photo/repository/PhotoRepository.java#L19-L112)

## 上传流程数据流

### 流程概述

上传流程从客户端发起POST /photos/upload请求开始，经过文件验证、存储空间检查、文件存储、缩略图生成和数据库持久化等步骤。**更新**：流程现在完全移除了用户认证和授权步骤。
...
**图表来源**
- [PhotoController.java:48-60](file://src/main/java/com/photo/controller/PhotoController.java#L48-L60)
- [PhotoService.java:50-110](file://src/main/java/com/photo/service/PhotoService.java#L50-L110)
- [FileStorageService.java:58-94](file://src/main/java/com/photo/service/FileStorageService.java#L58-L94)
...
- [PhotoService.java:50-110](file://src/main/java/com/photo/service/PhotoService.java#L50-L110)
- [FileStorageService.java:58-94](file://src/main/java/com/photo/service/FileStorageService.java#L58-L94)
- [FileUtils.java:106-118](file://src/main/java/com/photo/util/FileUtils.java#L106-L118)
- [ImageUtils.java:21-47](file://src/main/java/com/photo/util/ImageUtils.java#L21-L47)

## 下载流程数据流

### 流程概述

下载流程从客户端请求GET /photos/download/{filename}开始，系统先获取照片信息并更新下载计数，然后读取文件内容并返回给客户端。**更新**：流程现在移除了用户权限验证步骤，仅保留基础的文件存在性检查。
...
**图表来源**
- [PhotoController.java:149-178](file://src/main/java/com/photo/controller/PhotoController.java#L149-L178)
- [PhotoService.java:155-160](file://src/main/java/com/photo/service/PhotoService.java#L155-L160)
- [FileStorageService.java:98-114](file://src/main/java/com/photo/service/FileStorageService.java#L98-L114)
...
**图表来源**
- [PhotoController.java:154-178](file://src/main/java/com/photo/controller/PhotoController.java#L154-L178)
- [PhotoService.java:246-250](file://src/main/java/com/photo/service/PhotoService.java#L246-L250)
- [FileStorageService.java:98-114](file://src/main/java/com/photo/service/FileStorageService.java#L98-L114)
...
断点续传流程支持HTTP Range请求，允许客户端分段下载大文件。系统需要解析Range头，确定下载范围，然后读取指定范围的文件内容。**更新**：流程现在移除了用户权限验证步骤，仅保留基础的Range头解析和文件读取功能。
...
**图表来源**
- [PhotoController.java:183-224](file://src/main/java/com/photo/controller/PhotoController.java#L183-L224)
- [FileStorageService.java:227-256](file://src/main/java/com/photo/service/FileStorageService.java#L227-L256)
...
**图表来源**
- [PhotoController.java:196-211](file://src/main/java/com/photo/controller/PhotoController.java#L196-L211)
- [FileStorageService.java:227-256](file://src/main/java/com/photo/service/FileStorageService.java#L227-L256)
...
**图表来源**
- [PhotoController.java:36-44](file://src/main/java/com/photo/controller/PhotoController.java#L36-L44)
- [PhotoService.java:38-46](file://src/main/java/com/photo/service/PhotoService.java#L38-L46)
- [FileStorageService.java:26-28](file://src/main/java/com/photo/service/FileStorageService.java#L26-L28)
...
**章节来源**
- [FileStorageProperties.java:20-65](file://src/main/java/com/photo/config/FileStorageProperties.java#L20-L65)
- [application.yml:54-98](file://src/main/resources/application.yml#L54-L98)
...
**图表来源**
- [PhotoService.java:302-336](file://src/main/java/com/photo/service/PhotoService.java#L302-L336)
- [FileStorageService.java:58-94](file://src/main/java/com/photo/service/FileStorageService.java#L58-L94)
...
系统实现了基于Referer的防盗链保护机制，防止外部网站直接链接访问照片资源：

```mermaid
flowchart TD
Request[客户端请求] --> CheckReferer{检查Referer头}
CheckReferer --> |未设置| AllowDirect[允许直接访问]
CheckReferer --> |已设置| ValidateDomain{验证域名}
ValidateDomain --> |在允许列表内| AllowAccess[允许访问]
ValidateDomain --> |不在允许列表内| DenyAccess[拒绝访问]
AllowDirect --> ProcessRequest[处理请求]
AllowAccess --> ProcessRequest
DenyAccess --> Return403[返回403 Forbidden]
ProcessRequest --> Continue[继续处理]
```
...
系统通过SecurityProperties类提供灵活的安全配置选项：

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| security.referer.enabled | true | 是否启用防盗链 |
| security.referer.allowed-domains | localhost, 127.0.0.1 | 允许访问的域名列表 |
| security.token.secret | your-secret-key | Token密钥 |
| security.token.expiration | 86400 | Token过期时间(秒) |
| security.cors.enabled | true | 是否启用CORS |
| security.cors.allowed-origins | http://localhost:3000, http://localhost:8080 | 允许的源 |
| security.cors.allowed-methods | GET, POST, PUT, DELETE | 允许的方法 |
| security.cors.allowed-headers | * | 允许的头部 |

**章节来源**
- [SecurityProperties.java:15-67](file://src/main/java/com/photo/config/SecurityProperties.java#L15-L67)
- [application.yml:119-145](file://src/main/resources/application.yml#L119-L145)

## 总结

本文档详细分析了照片管理系统的核心数据流，涵盖了上传、下载和断点续传三大业务流程。系统采用分层架构设计，通过清晰的职责分离实现了高效的数据处理能力。

**重要更新**：系统现已移除用户认证和授权流程，简化为基于文件名的访问控制和防盗链保护机制。这种变化使得系统更加轻量级，适用于不需要复杂用户管理的场景。
...
- **安全配置灵活**：支持从YAML配置文件动态加载安全设置

该系统为照片管理提供了稳定、高效的解决方案，能够满足现代应用对文件处理的各种需求。移除的认证流程使得系统更加简洁，适用于不需要复杂用户管理的场景。
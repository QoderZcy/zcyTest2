...
本项目是一个基于Spring Boot 3.2.0的企业级照片上传下载系统，现已扩展支持认证、笔记、博客功能。项目实现了完整的照片管理、用户认证、便签管理和博客发布功能，采用现代化的测试策略来确保代码质量和系统稳定性。
...
DataJpaTest["DataJpaTest<br/>数据层测试"]
@end
subgraph "测试工具"
...
| spring-boot-starter-data-jpa | 3.2.0 | 数据层测试支持 |
| commonmark | 0.15.2 | Markdown渲染测试 |
...
@end
subgraph "测试范围"
AuthController["认证控制器测试<br/>AuthControllerTest"]
NoteController["便签控制器测试<br/>NoteControllerTest"]
BlogController["博客控制器测试<br/>BlogControllerTest"]
AuthServices["认证服务测试<br/>CustomUserDetailsServiceTest"]
NoteServices["便签服务测试<br/>NoteServiceTest"]
BlogServices["博客服务测试<br/>BlogServiceTest"]
Utils["工具类测试<br/>FileUtilsTest<br/>SecurityUtilsTest"]
@end
Integration --> AuthController
Integration --> NoteController
Integration --> BlogController
Integration --> AuthServices
Integration --> NoteServices
Integration --> BlogServices
Unit --> Utils
```

**图表来源**
- [AuthController.java](file://src/main/java/com/photo/controller/AuthController.java#L23-L111)
- [NoteController.java](file://src/main/java/com/photo/controller/NoteController.java#L18-L162)
- [BlogController.java](file://src/main/java/com/photo/controller/BlogController.java#L18-L173)
...
AuthController负责处理用户登录、注册和注销功能，测试策略重点关注安全性和用户体验。

##### 测试重点

1. **登录流程验证**：用户名密码验证、认证成功/失败场景
2. **注册流程验证**：用户创建、重复用户名处理、密码验证
3. **注销流程验证**：安全上下文清理、会话失效
4. **安全上下文管理**：Spring Security集成测试
...
participant AuthService as "CustomUserDetailsService"
Test->>MockMvc : POST /auth/login
MockMvc->>AuthController : 登录请求
AuthController->>AuthService : 验证用户凭据
AuthService-->>AuthController : 认证结果
AuthController-->>MockMvc : 返回响应
MockMvc-->>Test : 验证响应
```
...
| 测试方法 | 测试目标 | 覆盖功能 |
|---------|----------|----------|
| testLogin_Success | 登录成功 | POST /auth/login |
| testLogin_InvalidCredentials | 登录失败 | POST /auth/login |
| testLogout_Success | 注销成功 | POST /auth/logout |
| testRegister_Success | 注册成功 | POST /auth/register |
| testRegister_DuplicateUsername | 重复用户名 | POST /auth/register |
...
#### NoteController - 便签控制器测试

NoteController处理便签的CRUD操作和Markdown渲染功能，测试策略重点关注数据完整性和用户体验。

##### 测试重点

1. **CRUD操作验证**：创建、读取、更新、删除便签
2. **表单验证**：标题和内容的长度限制
3. **Markdown渲染**：内容到HTML的正确转换
4. **重定向处理**：成功/失败后的页面跳转
...
flowchart TD
Setup["测试准备<br/>@BeforeEach"]
CreateNote["创建便签<br/>POST /notes"]
ReadNote["读取便签<br/>GET /notes/{id}"]
UpdateNote["更新便签<br/>POST /notes/{id}"]
DeleteNote["删除便签<br/>POST /notes/{id}/delete"]
Setup --> CreateNote
CreateNote --> ReadNote
ReadNote --> UpdateNote
...
**图表来源**
- [NoteController.java](file://src/main/java/com/photo/controller/NoteController.java#L53-L149)

##### 测试用例分布

| 测试方法 | 测试目标 | 覆盖功能 |
|---------|----------|----------|
| testCreateNote_Success | 创建便签成功 | POST /notes |
| testCreateNote_InvalidInput | 无效输入处理 | POST /notes |
| testGetNote_Success | 获取便签详情 | GET /notes/{id} |
| testGetNote_NotFound | 便签不存在 | GET /notes/{id} |
| testUpdateNote_Success | 更新便签成功 | POST /notes/{id} |
| testDeleteNote_Success | 删除便签成功 | POST /notes/{id}/delete |
...
#### BlogController - 博客控制器测试

BlogController处理博客的CRUD操作、分类筛选和作者筛选功能，测试策略重点关注数据查询和过滤逻辑。

##### 测试重点

1. **CRUD操作验证**：博客的创建、读取、更新、删除
2. **筛选功能验证**：按分类和作者的博客列表筛选
3. **Markdown渲染**：博客内容的HTML转换
4. **URL路由验证**：RESTful API的正确路由

##### 关键测试场景

```mermaid
flowchart TD
BlogList["博客列表<br/>GET /blogs"]
FilterByCategory["按分类筛选<br/>GET /blogs?category=tech"]
FilterByAuthor["按作者筛选<br/>GET /blogs?author=admin"]
CreateBlog["创建博客<br/>POST /blogs"]
ViewBlog["查看博客<br/>GET /blogs/{id}"]
UpdateBlog["更新博客<br/>POST /blogs/{id}"]
DeleteBlog["删除博客<br/>POST /blogs/{id}/delete"]
BlogList --> FilterByCategory
BlogList --> FilterByAuthor
FilterByCategory --> ViewBlog
FilterByAuthor --> ViewBlog
CreateBlog --> ViewBlog
ViewBlog --> UpdateBlog
...
**图表来源**
- [BlogController.java](file://src/main/java/com/photo/controller/BlogController.java#L30-L171)

##### 测试用例分布

| 测试方法 | 测试目标 | 覆盖功能 |
|---------|----------|----------|
| testListBlogs_Success | 获取博客列表 | GET /blogs |
| testListBlogs_ByCategory | 按分类筛选 | GET /blogs?category=tech |
| testListBlogs_ByAuthor | 按作者筛选 | GET /blogs?author=admin |
| testCreateBlog_Success | 创建博客成功 | POST /blogs |
| testGetBlog_Success | 获取博客详情 | GET /blogs/{id} |
| testUpdateBlog_Success | 更新博客成功 | POST /blogs/{id} |
| testDeleteBlog_Success | 删除博客成功 | POST /blogs/{id}/delete |
...
NoteService提供便签的核心业务逻辑，包括Markdown渲染和数据转换功能。

##### 测试重点

1. **业务逻辑验证**：CRUD操作的正确性
2. **Markdown处理**：内容渲染和摘要生成
3. **数据转换**：实体到DTO的转换
4. **事务管理**：@Transactional注解的正确性
...
**图表来源**
- [NoteService.java](file://src/main/java/com/photo/service/NoteService.java#L115-L170)

##### 测试用例分布

| 测试方法 | 测试目标 | 覆盖功能 |
|---------|----------|----------|
| testCreateNote_Success | 创建便签成功 | NoteService.createNote |
| testUpdateNote_Success | 更新便签成功 | NoteService.updateNote |
| testRenderMarkdown_Success | Markdown渲染成功 | NoteService.renderMarkdown |
| testConvertToDTO_Success | DTO转换成功 | NoteService.convertToDTO |
...
BlogService提供博客的核心业务逻辑，包括多维度查询和Markdown处理功能。

##### 测试重点

1. **查询逻辑验证**：按分类、作者的博客查询
2. **业务逻辑验证**：CRUD操作的正确性
3. **Markdown处理**：内容渲染和摘要生成
4. **数据转换**：实体到DTO的转换
...
BlogDTO["BlogDTO对象"]
BlogEntity --> QueryConditions
QueryConditions --> FilteredResults
FilteredResults --> BlogDTO
BlogEntity --> BlogDTO
```

**图表来源**
- [BlogService.java](file://src/main/java/com/photo/service/BlogService.java#L55-L210)

##### 测试用例分布

| 测试方法 | 测试目标 | 覆盖功能 |
|---------|----------|----------|
| testGetBlogsByCategory_Success | 按分类查询成功 | BlogService.getBlogsByCategory |
| testGetBlogsByAuthor_Success | 按作者查询成功 | BlogService.getBlogsByAuthor |
| testCreateBlog_Success | 创建博客成功 | BlogService.createBlog |
| testRenderMarkdown_Success | Markdown渲染成功 | BlogService.renderMarkdown |
| testConvertToDTO_Success | DTO转换成功 | BlogService.convertToDTO |
